 ...
 * @author Thiago
 */
public class FpesqEntradaInterna extends javax.swing.JDialog {
    ...
    public FpesqEntradaInterna(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    {
        String vmCampos = "ent_codigo_id, emp_nome, par_nome_razao_social, ent_data, ent_controle_interno";
      
        String vmCondicao_Consulta = "";
        if (!txtCodigo.getText().equals(""))
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = ent_par_codigo_id) " +
                                  " left join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' and ent_codigo_id =  " + txtCodigo.getText() + " order by ent_codigo_id";
        else
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = ent_par_codigo_id)" +
                                  " left join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' order by ent_codigo_id";
        ...
        while (rs.next())
        {
            dtm.addRow(new Object[]{rs.getString("ent_codigo_id"),rs.getString("emp_nome"),rs.getString("par_nome_razao_social"),funcoes.formataData(rs.getString("ent_data")),rs.getString("ent_controle_interno"),""});
        }
        rs.close();
    ...
    public void MontaGridDescricao()
    {
        ...
        if (!txtDescricao.getText().equals(""))
            vmCondicao_Consulta = " inner join parceiros on (par_codigo_id = ent_par_codigo_id) " +
                                  " inner join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' and emp_nome like '" + txtDescricao.getText() + "%' order by ent_codigo_id";
        else
            vmCondicao_Consulta = " inner join parceiros on (par_codigo_id = ent_par_codigo_id)" +
                                  " inner join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' order by ent_codigo_id";
   ...
   public void MontaGridControleInterno()
    {
        ...
        if (!txtControleInterno.getText().equals(""))
            vmCondicao_Consulta = " inner join parceiros on (par_codigo_id = ent_par_codigo_id) " +
                                  " inner join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' and ent_controle_interno = '" + txtControleInterno.getText() + "' order by ent_codigo_id";
        else
            vmCondicao_Consulta = " inner join parceiros on (par_codigo_id = ent_par_codigo_id) " +
                                  " inner join empresas on (emp_codigo_id = ent_emp_codigo_id) where par_tipo_parceiro = 'FORNECEDOR' order by ent_codigo_id";
    ...
    private void initComponents() {
        ...
        setTitle("Pesquisa Entrada Interna");
        setResizable(false);
