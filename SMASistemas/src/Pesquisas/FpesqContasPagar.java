 ...
 * @author Thiago
 */
public class FpesqContasPagar extends javax.swing.JDialog {
    ...
    public FpesqContasPagar(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...

    {
        ...
        String vmCampos = "cp_codigo_id, par_nome_razao_social, emp_nome, cp_documento, cp_valor_documento, cp_data_emissao, cp_data_vencimento";
      
        String vmCondicao_Consulta = "";
        if (!txtDocumento.getText().equals(""))
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = cp_par_codigo_id) " +
                                  " left join empresas on (emp_codigo_id = cp_emp_codigo_id) where cp_documento = '" + txtDocumento.getText() + "' order by cp_documento";
        else
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = cp_par_codigo_id) inner join empresas on (emp_codigo_id = cp_emp_codigo_id) order by cp_codigo_id desc";
        ...
        while (rs.next())
        {
            dtm.addRow(new Object[]{rs.getString("cp_codigo_id"),rs.getString("emp_nome"),rs.getString("par_nome_razao_social"),rs.getString("cp_documento"),funcoes.formataData(rs.getString("cp_data_emissao")),funcoes.formataData(rs.getString("cp_data_vencimento")),funcoes.formataMoeda(rs.getString("cp_valor_documento"),"BD"),""});
        }
        rs.close();
    ...
    public void MontaGridDescricao()
    {
        ...
        if (!txtParceiro.getText().equals(""))
            vmCondicao_Consulta = " LEFT join parceiros on (par_codigo_id = cp_par_codigo_id) " +
                                  " LEFT join empresas on (emp_codigo_id = cp_emp_codigo_id) where par_nome_razao_social like '" + txtParceiro.getText() + "%' order by par_nome_razao_social";
        else
            vmCondicao_Consulta = " LEFT join parceiros on (par_codigo_id = cp_par_codigo_id) " +
                                  " LEFT join empresas on (emp_codigo_id = cp_emp_codigo_id) order BY par_nome_razao_social";
    ...
    public void MontaGridData()
    {
        ...
        if (!txtVencimento.getText().equals("  /  /    "))
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = cp_par_codigo_id) " +
                                  " left join empresas on (emp_codigo_id = cp_emp_codigo_id) where cp_data_vencimento = '" + txtVencimento.getText() + "' order by cp_data_vencimento";
        else
            vmCondicao_Consulta = " left join parceiros on (par_codigo_id = cp_par_codigo_id) " +
                                  " left join empresas on (emp_codigo_id = cp_emp_codigo_id) order by cp_data_vencimento";
    ...
    private void initComponents() {
        ...
        setTitle("Pesquisa Contas a Pagar");
        setResizable(false);
